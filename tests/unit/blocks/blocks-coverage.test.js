...
const BLOCKS_TODO = new Set([
  ...
  'Physics', // physics-sim signal sources (ADR 059) — factory + closure/route heavy; text-first, blocks pending
...
describe('blocks coverage gate', () => {
  ...
  it('the buckets are disjoint', () => {
    for (const name of TOOLKIT_NAMES) {
      const inN =
        (name in COVERAGE ? 1 : 0) +
        (TEXT_ONLY_INTENTIONAL.has(name) ? 1 : 0) +
        (BLOCKS_TODO.has(name) ? 1 : 0);
