...
const _toneMock = {
  ...
  Frequency: (v) => ({
    toMidi: () => {
      const n = { C4: 60, D4: 62, E4: 64, G4: 67, Bb3: 58, C3: 48, E3: 52, G3: 55 };
      return n[v] ?? 60;
    },
    toFrequency: () => 440,
  }),
...
global.navigator = {
  mediaDevices: { enumerateDevices: async () => [], getUserMedia: async () => ({}) },
};
  ...
  function parseItems(end) {
    ...
    while (pos < tokens.length) {
      ...
      if (t === '[') {
        ...
        if (rm) {
          repeat = +rm[1];
          val = val.slice(0, -rm[0].length);
        }
        const im = val.match(/!(\d*)$/);
        if (im) {
          repeat = +im[1] || 2;
          val = val.slice(0, -im[0].length);
        }
        const wm = val.match(/@(\d*\.?\d+)$/);
        if (wm) {
          weight = +wm[1];
          val = val.slice(0, -wm[0].length);
        }
        const dm = val.match(/\?(\d*\.?\d*)$/);
        if (dm) {
          degrade = dm[1] ? +dm[1] : 0.5;
          val = val.slice(0, -dm[0].length);
        }
...
function atomValues(items) {
  ...
  for (const it of items) {
    if (it.type === 'atom' && it.value !== '~' && it.value !== '.') {
      ...
      vals.push(...atomValues(it.items));
    } else if (it.type === 'poly') {
      for (const g of it.groups) vals.push(...atomValues(g));
    }
  }
  return vals;
}
...
function flatten(items, cycleNum = 0) {
  ...
  for (const it of items) {
    ...
    if (it.type === 'atom') {
      ...
      flatten(it.items, cycleNum).forEach((e) =>
        events.push({ ...e, time: e.time * slot + off, dur: e.dur * slot }),
      );
      off += slot;
    } else if (it.type === 'alt') {
      if (it.items.length) {
        const chosen = it.items[cycleNum % it.items.length];
        flatten([chosen], cycleNum).forEach((e) =>
          events.push({ ...e, time: e.time * slot + off, dur: e.dur * slot }),
        );
      }
      off += slot;
      ...
      for (const grp of it.groups) {
        flatten(grp, cycleNum).forEach((e) =>
          events.push({ ...e, time: e.time * slot + off, dur: e.dur * slot }),
        );
      }
      off += slot;
...
function query(str, cycleNum = 0) {
  return flatten(parse(tokenize(str)), cycleNum);
}
...
describe('tokenizer', () => {
  ...
  it('groups and alt', () =>
    expect(tokenize('[C4 E4] <G4 Bb4>')).toEqual(['[', 'C4', 'E4', ']', '<', 'G4', 'Bb4', '>']));
