...
### Editor Window
The floating window that holds the code area (text — the sole modality since ADR 060 retired blocks), play/pause/stop controls, and embedded console. Each Editor Window belongs to exactly one Editor Instance. Cannot be closed without confirmation; can be minimized to the Taskbar.
...
### Widget Restorer Registry
The third **register-beside-your-code** registry (`src/api/wm/widget-restorer-registry.js`), a sibling to the **Window Type Adapter** and **Desktop File-Type Adapter**: `registerWidgetRestorer(type, fn)` records how a code-generated widget window (`drumpad`/`piano`/`note`/`paint`/`spriteEditor`/`ascii`/`mixer`) is rebuilt from a saved WM record, called at module load **beside each widget class**. `wm.restoreState` resolves a `widgetType` through `restoreWidget(type, state)`. Replaces the bare `window.__ar_widgetRestorers` global object literal that app.js built inline in `window.onload` (~130 lines) and that reached into widget privates (`sp._frames`, `sp._render()`) — sprite restore now paints through the public `Sprite.drawImageToFrame`. Same discipline as `onReset` / `registerWindowType` / `registerDesktopFileType`.
_Avoid_: widget restorer global, `__ar_widgetRestorers`, restore switch.

### Trigger Surface Chassis
The shared lifecycle wiring behind every **Trigger Surface** (ADR 056), extracted as **composition helpers** — not a base class (ADR 007 discipline) — in `src/api/audio/trigger-surface.js`. ADR 046 shared the **Binding** and **Voice** model; the chassis shares everything *around* it that Piano/Drumpad/Launchpad used to copy three ways: the output bus summed into one window-scoped **Strip**, the `BindingMap` voice→bus router, the MIDI register/notify lifecycle, the Performance **Take**, and the mirror-image teardown (`initTriggerSurface` / `enableSurfaceMidi` / `detachTriggerSurface` / `disposeTriggerSurface`). What stays per-surface is the part that genuinely differs — the **sound core** of the strike path (attack/release vs one-shot vs chopped slice), key resolution, UI, and synth ownership. The strike **envelope** around that core — the `isSilent` gate, the Action bus-event dispatch, and the post-hook ordering — is itself shared via `strikeCore(self, key, { sound, payload, after })` (deepening pass #5, refining ADR 056): the surface passes its sound/payload/after as **closures**, so the ordering lives once with no per-surface branch inside the helper. The byte-identical `replay(actions, opts)` is installed on `self` by `initTriggerSurface` (only `_applyAction` differed).
...
### API Descriptor
The single record of everything a built-in API *is*, carried by its registration in `src/runtime/api-registry.js`: the implementation, plus a `descriptor` holding its parameter signatures (`params`), authoring-surface metadata, and a detection spec (`detect: { effect, triggers? }`). The descriptor is the **source of truth** for the surfaces that read it rather than re-declaring it: `PARAM_HINTS` (`param-hints.js`) defers to `deriveParamHints()`, and — since ADR 058 — audio-usage detection defers to `deriveAudioDetectPattern()`. `detect.effect` is `'audio'` or absent; the only effect wired is **audio-start** (ADR 058: `usesAudio` drives audio pre-warm + the taskbar audio chip + the `await __ar_audioReady` execute-gate). There is no `'canvas'` effect — ADR 040 deleted the auto-opened output window, so visual APIs self-spawn and are not detected at all. The audio trigger is derived from the **registered name** (so a rename can't silently break detection): capitalised → `\bnew Name\b`, lower-case → `\bname\s*[.(]`, plus optional literal `triggers[]` (Strudel's universal `.play()`). Correctness does **not** rest on detection — silence is made impossible structurally by the lazy gesture-unlock at `acquireStrip` (ADR 058); detection is a pre-warm/UX signal only. The one authoring surface that reads the descriptor's editorial metadata is the curated `TOOLKIT_CATEGORIES` snippet drawer (label/code/hint) — the Blockly `blocks:` extension was removed with ADR 060. `KNOWN_GLOBALS` (the completion-coherence test mirror) is still derived by grepping `register-builtins.js` for `_registerBuiltin` names, not by reading the registry object. ADR 058 superseded the audio half of ADR 012's central-table detection gate.
...
### Run Context
The single owner of the handful of **per-run lifecycle fields** that the whole codebase reads to know "what's running right now" (`src/runtime/run-context.js`): the **Active Editor** id, the `paused` flag, and the audio-state flags (`usesAudio`, `audioReady`, `friendlyError`). Replaces ~6 of the raw `window.__ar_*` globals that a dozen API files used to read and several wrote directly, with no accessor — typed getters/setters (`activeEditorId()`/`setActiveEditorId()`, `isPaused()`/`setPaused()`, …) over one owner, the way `keep-alive.js` already fronts `__ar_keepAlive`. Deliberately **narrow**: it owns lifecycle state only — the app-wiring factory handles (`__ar_instances`, `__ar_projectManager`…) and the device singletons stay with their own modules, since they have a different lifetime (boot-time composition / subsystem-owned) than per-run state. The migration is now **complete**: every reader of `active_editor_id`/`paused` crosses the accessor seam (the owner-capture in route/viz/canvas/camera/three-scene/pipeline constructors, the paused-raf-gate in input/sensors/shader/pixi/vision loops); the `window.__ar_*` fields survive only as the private backing store. The **Mic device singleton** got the same treatment separately — a **Mic accessor** (`mic-state.js`, `micAnalyser`/`micStream`/`micIsOn`/`micViz`) now fronts `window.__ar_mic_*`, so `analyser-read`/`audio-viz`/`viz`/`shader`/`stt-engine` no longer read it raw.
_Avoid_: global state, the `__ar_*` bus, app-context (that conflates boot wiring with run state).

### Result-Before-Notation
The authoring thesis this IDE commits to (ADR 060, which retired Blockly): a beginner should get an expressive **result first** — play Paint/Drumpad, drag a number scrubber, manipulate a widget — and only then have that moment turned into real, runnable code as a *souvenir*, never be made to learn a notation before the reward. The opposite of Scratch-style *notation-before-result* (learn blocks, then produce). Visual authoring is delivered as **directional projections** of the canonical text — partial, one-way views kept in sync by deriving/applying patches — never a parallel, independently-editable mirror. Text stays the single source of truth. Delivered incrementally as a tier ladder: **Tier 0** token editors (scrubbers/swatches — built), **Tier 1** per-call **Inline Controls** (ADR 061/062), **Tier 2** the live **Chain Inspector** (ADR 063), domain widgets → **Snapshot**/**Performance** (Tier 3 — built). Each tier is its own ADR.
_Avoid_: blocks-first, visual-before-textual, parallel modality, blocks mode.

### Inline Control
The Tier 1 authoring surface (ADR 061): a **bespoke, semantic** editing widget scoped to **one whole call's arguments** inside a recognized `route()`/`pipe()` chain — e.g. `.scale(0,1,0,100)`'s four numbers rendered as *two coupled ranges* (input→output), not four unrelated scrubbers. Distinct from a **Tier 0** scrubber/swatch (one literal token, generic) and a **Tier 2** chain inspector (the whole chain, read-only). Registered **editor-side** (`src/editor/inline-controls.js`, `registerInlineControl(headType, method, builder)`) because it is CodeMirror DOM — never beside the runtime API, which must not import `@codemirror/view`. The matcher **anchors on the chain head** (walks `callee.object` to a base `route(`/`pipe(` call) so a control appears only inside a real signal chain, never on an unrelated `.scale`. Attaches **all-or-nothing** (only when every edited arg is a literal; else Tier 0 scrubbers cover the literals), edits **per-arg atomically** (preserves formatting), and shows as an **inline glyph → popup** (reusing the colour-picker popup). Project-shipped for built-in methods; a user's custom `pipe.register` stage gets none (falls back to Tier 0). A directional **projection** of the canonical text — see **Result-Before-Notation**.

Two mechanisms: **(1) positional/bespoke** (ADR 061) — a hand-authored control registered per `head.method` (`route.scale`'s coupled two-range editor), whole-control all-or-nothing on its numeric-literal args. **(2) opts-object/inferred** (ADR 062, the ⚙ glyph) — for a chain call whose sole arg is an object literal (`pipe(cam).pixelate({blockSize:8})`), fields are **inferred** from the object's properties (value-literal type → number input / colour swatch / text / checkbox), zero metadata, per-property (only literal-valued props become fields). The inferred variant deliberately **reverses** the "bespoke not generic" stance — justified because pipe opts get no Tier 0 coverage (Tier 0 skips the object arg), so it adds capability rather than a redundant box, and it gives custom `pipe.register` stages an opts editor for free. `chainHead` is the shared matcher for both.
_Avoid_: micro-widget, arg panel, generic field editor, block.

### Demo Gallery
The toolbar-opened WM window of runnable example projects (`public/demos/*.vljson`, indexed by `index.json`). Organized **by outcome, not API category** (ADR 064): each demo declares a primary **`goal`** — one of a curated 7-bucket, beginner-verb taxonomy (React to your face & hands / React to sound / Make music & sound / Make it move / Play with it / Camera effects / Paint & pixels) — and the gallery renders a headed section per non-empty goal (`groupDemos`/`GOAL_ORDER`/`GOAL_LABELS` in `demo-gallery.js`). This is the first outcome-first discovery surface, the entry-point expression of **Result-Before-Notation**: a beginner picks what they want to *make*, not which API to learn. `goal` is a single editorial choice (not derived from the demo's many `tags`, which stay for future search); an integrity test forbids a demo with an unknown/missing goal.
_Avoid_: by-category gallery, tag-derived sections.

### Chain Inspector
The Tier 2 surface (ADR 063): `route(...).watch(title?)` opens a WM window rendering that route's chain as **role-tagged nodes** (source/transform/combinator/sink/listener; unknown ops render **opaque** — a muted node that still shows its name + value, never a collapsed gap) with **live per-stage values and active-stage lighting**, RAF-updated and **read-only**. Unlike a Tier 1 **Inline Control** (an editor decoration on the *text*), it is a runtime panel reading the **live route object** (`_src`/`_chain`/`_sinks`) — a per-chain, detailed sibling of `signalGraph.show()`'s whole-graph view. Per-stage values come from `_eval` instrumentation recorded **only while `_inspecting`**. If the watched route has no sink, `watch()` adds a no-op driver and owns the route (close → `_destroy`); otherwise close just stops watching. The method is **`watch`, not `inspect`** — `inspect` collides with the Node/pretty-format custom-inspect protocol and, returning `this`, recurses. Frame routes (camera/pipe) show structure only (no scalar values) in v1.
_Avoid_: inspect (reserved name — recursion trap), debugger, signal graph (that is the whole-program view).
...
### Physics Source
A numerical **simulation** exposed as a source category, distinct from the shader/pattern **visualizers** (a visualizer *renders* a signal; a Physics Source *generates* one). A sim is registered **beside its own code** via `registerPhysicsSource` — the sixth register-beside-your-code registry, a sibling of `registerSource`/`registerWindowType`/`registerDesktopFileType`/`registerWidgetRestorer` — and instantiated by the user through the `physics(name, opts)` factory. It is **dual-character**: it publishes **continuous channels** (each a routable scalar reader such as an angle, angular velocity, energy, or order parameter) and **discrete events** (bounces, zero-crossings, flips — named bus events). Continuous channels are consumed by `route()` as plain callable readers (no route change); events by `route('event')`/`on('event')`. Every live sim is advanced by **one shared fixed-step clock** owned by the registry (a fixed internal dt with a substep accumulator, so integration is frame-rate-independent and chaotic sims stay stable), gated on the run's paused state. Like **Canvas**, a Physics Source **survives soft reset by identity** (keyed by `id` or opts-signature) so live-coding does not snap a chaotic trajectory back to its initial conditions on every keystroke; it is torn down a cycle after it drops from the code, and on hard reset / stop / debug-window close.
_Avoid_: physics engine (there is no rigid-body world — each sim is its own integrator), particle system, visualizer (that renders; this generates).

### Signal Channel
One continuous output of a **Physics Source** — a single named scalar (`theta2`, `energy`, an order parameter `R`) exposed as a callable reader routed like any other continuous source. A sim declares its channels (with an optional hint range for normalization) in its registration; a multi-channel sim publishes several, each independently routable, so one sim drives several sinks at different mappings. Contrast a **discrete event**, the sim's other output shape.
...
### Editor Persistence
Each Editor Instance saves its code to `localStorage` under key `vl-ide-code-{id}`. A manifest key `vl-ide-editors` holds the ordered list of active editor IDs. On page load all editors in the manifest are recreated with their saved code. All these key literals live in one place now — `storage-keys.js` (frozen values, per-editor keys as functions). For a **project** round-trip the editor owns its half of the record: `EditorInstance.serialize()` → `{ code, mode, executionState }` and `applyRecord(rec)` (the inverse), so `project.js` / `project-manager.js` compose only the window-owned fields (title/geo/audio/editorId) instead of each hand-listing the editor fields and reaching into `inst.cm`. (`mode` is always `'text'` since ADR 060; the field is kept for record-shape stability.)
